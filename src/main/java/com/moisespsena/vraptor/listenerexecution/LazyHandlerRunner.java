 ...
 */
public class LazyHandlerRunner<T> implements HandlerRunner<T> {
	...
	public LazyHandlerRunner(final ListenerInstanceResolver instanceResolver,
	...
	private T getAcceptor() throws ExecutionStackException {
		if (acceptor == null) {
			acceptor = instanceResolver.instanceForWithoutDependencies(type);
		}
		return acceptor;
	}
	 ...
	 * 
	 * @see
	 * com.moisespsena.vraptor.listenerexecution.HandlerRunner#isInterception()
	 */
	@Override
	...
	public void run(final ExecutionStack<T> stack,
		...
		final T acceptor = getAcceptor();

		if (listenerExecutor.accepts(acceptor)) {
			final T instance = instanceResolver.instanceFor(type);

			if (instance == null) {
				throw new ExecutionStackException(
						"Unable to instantiate interceptor for "
								+ type.getName()
